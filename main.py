...
def main():
    ...
    parser.add_argument("--epochs", type=int, default=400, help="Specifies the number of epochs to train.")
    parser.add_argument("--batch_size", type=int, default=128, help="Specifies the batch size to train the model.")
    parser.add_argument("--seed", type=int, default=1, help="Specifies the seed.")
    ...
    parser.add_argument("--gpu", type=str, default="/device:GPU:0", help="Specifies the gpu to train on. Default: /device:GPU:0")
    parser.add_argument("--dropout", type=float, default=0.4, help="Specifies the dropout ratio.")
    parser.add_argument("--sampling_strategy", type=str, default="hard", choices=["random", "semi_hard", "hard"], help="Specifies the sampling strategy.")
    ...
    parser.add_argument("--num_target_traces", type=int, default=1, help="Specifies the number of target traces per user.")
    ...
    parser.add_argument("--k", type=int, default=0, help="Specifies the number of observed traces per client.")
    parser.add_argument("--fields", type=list, nargs='+', help="Fields to consider", default=["state", "street", "postcode", "speed"])
    ...
    with tf.device(args.gpu):
        ...
        user_to_sample_idx_map = gen_user_to_sample_idx_map(user_to_traces_map, args.k)
        
        train_user_to_traces_map = {client: [e for i, e in enumerate(user_to_traces_map.get(client)) if i in traces_list] for client, traces_list in user_to_sample_idx_map.items()}
        train_num_samples = sum([len(v) for k, v in train_user_to_traces_map.items()])
        
        user_to_target_idx_map = gen_user_to_target_idx_map(args, user_to_traces_map)
        ...
        for epoch in range(args.epochs):
            ...
            if epoch % 2 == 0:
                ...
                for i in range(args.sample_size):
                    ...
                    target_trace_idx_list = user_to_target_idx_map.get(target_user)
                    for k, target_trace_idx in enumerate(target_trace_idx_list):
                        target_trace = user_to_traces_map.get(target_user)[target_trace_idx]
                        for p in range(0, num_users, args.batch_size):
                            if args.approach == "tl":
                                batch_x, _ = get_test_triplet_batch(args, user_to_traces_map, user_to_test_idx_map, target_trace, target_user, p)
                                ...
                                output = tf.reduce_mean(tf.square(batch_preds_anchor - batch_preds_positive), axis=1) 
                                preds.extend(list(output.numpy()))
                            else:
                                batch_x, _ = get_test_pair_batch(args, user_to_traces_map, user_to_test_idx_map, target_trace, target_user, p)
                ...
                for i in range(0, len(preds), num_users):
                    ...
                    j += 1
                logging.info("Top 1: %.4f Top 1 (Std): %.4f - Top 10: %.4f Top 10 (Std): %.4f - Top 10 Percent: %.4f Top 10 Percent (Std): %.4f" % (np.mean(in_top_1_list), np.std(in_top_1_list), np.mean(in_top_10_list), np.std(in_top_10_list), np.mean(in_top_10_percent_list), np.std(in_top_10_percent_list)))
